...
// Creates a random palindrome that includes a random ASCII punctuation /or space  every two random unicode chars
func randomPalindrome(rng *rand.Rand) string {
	...
	return createPunctuatedString(runes, rng)
}
...
const ASCIIPuntuation string = `.,/;:?><"'[]{}-_*~ `

func createPunctuatedString(runes []rune, rng *rand.Rand) string {
	var builder strings.Builder
	for _, v := range runes {
		builder.WriteRune(v)
		builder.WriteRune(choosePunctuation(rng))
	}
	return builder.String()
}

func choosePunctuation(rng *rand.Rand) rune {
	return rune(ASCIIPuntuation[rng.Intn(len(ASCIIPuntuation))])
}
