...
func TestReportCyclicStruct(t *testing.T) {
    a := &link{value: "a"}
    if !ReportCyclicStruct(a) {
        t.Errorf("expected report of cyclic data structure for :%+v", a)
    }
    b := struct{
        name string
    }{
    ...
    if ReportCyclicStruct(b) {
        t.Errorf("expected report of non-cyclic data structure for :%+v", b)
    }
    c := struct {
        name string
        l link
    ...
    if ReportCyclicStruct(c) {
        t.Errorf("expected report of cyclic data structure for :%+v", c)
    }
    if ReportCyclicStruct(int(6)) {
        t.Errorf("expected report of non-cyclic data structure for :%+v", int(6))
    }
